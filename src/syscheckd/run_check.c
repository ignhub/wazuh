...
STATIC void fim_link_delete_range(const directory_t *configuration);
STATIC void fim_link_silent_scan(const char *path, const directory_t *configuration);
    ...
    while (1) {
        ...
        foreach_array(dir_it, syscheck.directories) {
            if (dir_it->options & REALTIME_ACTIVE) {
                realtime_adddir(dir_it->path, 0, 0);
            }
        }
#endif
...
void set_whodata_mode_changes() {
    ...
    foreach_array(dir_it, syscheck.directories) {
        if (dir_it->dirs_status.status & WD_CHECK_REALTIME) {
            ...
            if (realtime_adddir(dir_it->path, 0, 0) != 1) {
